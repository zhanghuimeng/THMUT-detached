...
# Train a De-De encoder model, with encoder loaded and frozen
...
def parse_args(args=None):
    ...
    # Added, to load the encoder part
    parser.add_argument("--encoder_checkpoint", type=str,
                        help="Path to pre-trained translator model")
    parser.add_argument("--half", action="store_true",
                        help="Enable FP16 training")
    parser.add_argument("--frozen", action="store_true",
                        help="Enable frozen training")
...
def restore_encoder_variables(checkpoint):
    ...
    for var in var_list:
        ...
        if name in values:
            tf.logging.info("Loaded %s" % var.name)
            ops.append(tf.assign(var, values[name]))
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        # Load
        load_encoder_op = restore_encoder_variables(args.encoder_checkpoint)
