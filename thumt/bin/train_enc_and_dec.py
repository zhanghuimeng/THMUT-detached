...
# Load encoder and decoder separately from two trainers
# But has got some matrix in between...
...
def parse_args(args=None):
    ...
    parser.add_argument("--enc_ckpt", type=str,  # enc
...
def restore_encoder_variables(checkpoint):
    ...
    for var in var_list:
        ...
        if name in values:
            tf.logging.info("Loaded for encoder: %s" % var.name)
            ops.append(tf.assign(var, values[name]))

    return tf.group(*ops, name="load_encoder_op")
...
def restore_decoder_variables(checkpoint):
    ...
    for (name, shape) in var_list:
        if not ("transformer/target_embedding" in name or "transformer/decoder/" in
                name or "transformer/softmax" in name):
            continue
        tensor = reader.get_tensor(name)
        name = name.split(":")[0]
        values[name] = tensor
    ...
    for var in var_list:
        ...
        if name in values:
            tf.logging.info("Loaded for decoder: %s" % var.name)
            ops.append(tf.assign(var, values[name]))

    return tf.group(*ops, name="load_decoder_op")
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        if params.adapt_mode != "frozen":
            ...
            # Compress ValueError: No variables to optimize.
            train_op = tf.assign_add(global_step, 1, name='increment')
        ...
        load_encoder_op = restore_encoder_variables(args.enc_ckpt)
        load_decoder_op = restore_decoder_variables(args.dec_ckpt)
