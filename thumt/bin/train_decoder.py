#!/usr/bin/env python
# coding=utf-8
# Train THUMT decoder with the encoder (English) outputs.
...
def parse_args(args=None):
    parser = argparse.ArgumentParser(
        description="Training NMT decoder with (Deutsch) representation and sentence",
        usage="trainer.py [<args>] [-h | --help]"
    )

    # input files
    parser.add_argument("--input", type=str, nargs=2,
                        help="Path of representation and sentence")
...
def override_parameters(params, args):
    ...
    params.vocabulary = {
        "target": vocabulary.load_vocabulary(params.vocab)
    }
...
def main(args):
    ...
    with tf.Graph().as_default():
        if not params.record:
            # Build input queue
            features = dataset.get_decoder_training_input(params.input, params)
        else:
        ...
        sharded_losses = parallel.parallel_model(
            ...
            model.get_decoder_training_func(initializer, regularizer, dtype),
            features,
            params.device_list
        )
        ...
        if params.validation and params.references[0]:
            ...
            eval_inputs = dataset.sort_and_zip_files_for_decoder(files)
            eval_input_fn = dataset.get_evaluation_input_decoder
        else:
            eval_input_fn = None
