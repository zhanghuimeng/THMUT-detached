...
# Changed encoder to bert and fix it.
...
def parse_args(args=None):
    ...
    parser.add_argument("--vocabulary", type=str, nargs=2,
                        help="Path of source and target vocabulary. The source is the "
    ...
    parser.add_argument("--init_bert_checkpoint", type=str, default=None,
...
def override_parameters(params, args):
    ...
    params.bert_config = modeling.BertConfig.from_json_file(args.bert_config_file)
...
def restore_bert_variables(checkpoint):
    ...
    for var in var_list:
        name = var.name.split(":")[0]
        if not name.startswith("transformer/bert/"):
            continue

        stripped_name = name[12:]
...
def main(args):
    ...
    with tf.Graph().as_default():
        if not params.record:
            # Build input queue
            features = dataset.get_bert_encoder_training_input(params.input, params)  # bert!
        else:
            ...
            tf.train.LoggingTensorHook(
                {
                    "step": global_step,
                    "loss": loss,
                    "source": tf.shape(features["input_ids"]),
        ...
        load_bert_op = restore_bert_variables(args.init_bert_checkpoint)
