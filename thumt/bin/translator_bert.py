...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Translate using existing BERT NMT models",
...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        features = dataset.get_inference_input_bert(sorted_inputs, params)  # bert!
        # Create placeholders
        placeholders = []
        ...
        for i in range(len(params.device_list)):
            placeholders.append({
                "input_ids": tf.placeholder(tf.int32, [None, None], "input_ids_%d" % i),
                "source_length": tf.placeholder(tf.int32, [None], "source_length_%d" % i),
                "input_type_ids": tf.placeholder(tf.int32, [None, None], "input_type_ids_%d" % i),
                "input_mask": tf.placeholder(tf.int32, [None, None], "input_mask_%d" % i),
            })
        ...
        if params.generate_samples:
            ...
            inference_fn = inference.create_inference_graph_bert  # bert!
