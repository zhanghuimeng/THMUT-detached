...
def get_bert_encoder_training_input(filenames, params):
    """ Get input for training stage
    Copied from get_training_input.
    Modified by bert/extracted_features.py.
    It is bert encoder input.
    ...
    with tf.device("/cpu:0"):
        ...
        dataset = dataset.map(
            lambda src, tgt: {
                ...
                "source_length": tf.shape(src),
                "target_length": tf.shape(tgt),
                "input_type_ids": tf.zeros([tf.shape(src)[0]], dtype=tf.int32),
                "input_mask": tf.ones([tf.shape(src)[0]], dtype=tf.int32),
            },
            num_parallel_calls=params.num_threads
        )
        ...
        # Convert to int32
        features["input_ids"] = tf.to_int32(features["input_ids"])
        features["target"] = tf.to_int32(features["target"])
        features["source_length"] = tf.to_int32(features["source_length"])
        features["target_length"] = tf.to_int32(features["target_length"])
        features["source_length"] = tf.squeeze(features["source_length"], 1)
