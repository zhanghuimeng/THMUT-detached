...
# Copied from transformer.py, to do encoder-frozen training
...
def _layer_process(x, mode, frozen=False):
    if not mode or mode == "none":
        return x
    elif mode == "layer_norm":
        return layers.nn.layer_norm(x, frozen=frozen)
    else:
            ...
            with tf.variable_scope("layer_%d" % layer):
                with tf.variable_scope("self_attention"):
                    ...
                    y = layers.attention.multihead_attention(
                        _layer_process(x, params.layer_preprocess, frozen=True),
                        None,
                        bias,
                        params.num_heads,
                    ...
                    x = _layer_process(x, params.layer_postprocess, True)
